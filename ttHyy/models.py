...
def model_rnn_2(indata_jets, indata_photons):
    m_in_jets = Input(shape=indata_jets[0].shape, name='in_jets')
    m_jets = Masking(mask_value=-999)(m_in_jets)
    m_jets = LSTM(16, return_sequences=True)(m_jets)

    m_in_photons = Input(shape=indata_photons[0].shape, name='in_photons')
    m_photons = Masking(mask_value=-999)(m_in_photons)
    m_photons = LSTM(16, return_sequences=True)(m_photons)

    m = concatenate([m_photons, m_jets], axis=1)
    m = LSTM(16)(m)
    m = Dense(32)(m)
    m = Activation('relu')(m)
    m = Dropout(0.2)(m)
