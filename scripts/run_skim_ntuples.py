...
def arrange_cmd(channel,category,inputdir, skip):
    ...
    for filename in os.listdir(inputdir):
        ...
        if os.path.isfile('skimmed_ntuples/%s/%s' % (category, filename)):
            ...
            try:
                if (t0.GetEntries("(PassesDiMuonSelection == 1) && Muons_Minv_MuMu_OnlyNearFsr >= 110 && SampleOverlapWeight && EventWeight_MCCleaning_5") == t1.GetEntries()) and (t1.GetEntries() == t10.GetEntries() + t11.GetEntries() + t12.GetEntries()):
                        continue
                else:
