...
def gettingsig(region, variable, boundaries_VBF, boundaries, transform, estimate):
    ...
    for category in ['sig', 'VBF', 'ggF', "data_sid", "bkgmc_sid", "bkgmc_cen"]:
        ...
        for data in tqdm(read_root(f'outputs/{region}/{"bkgmc" if "bkgmc" in category else category}.root', key='test', columns=[f"{variable}_score{'_t' if transform else ''}", f"{variable}_score_VBF{'_t' if transform else ''}", 'm_mumu', 'weight', 'eventNumber'], chunksize=500000), desc=f'Loading {category}', bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}'):
...
def categorizing(region, variable, sigs, bkgs, nscan, nscanvbf, minN, transform, nbin, nvbf, floatB, n_fold, fold, earlystop, estimate):
    ...
    if estimate in ["fullSim", "fullSimrw"]:
        ...
        t_bkgmc.Draw(f"{variable}_score{'_t' if transform else ''}:{variable}_score_VBF{'_t' if transform else ''}>>h_bkgmc_cen_raw", "weight*%f*((m_mumu>=120&&m_mumu<=130)&&(eventNumber%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
        t_bkgmc.Draw(f"{variable}_score_VBF{'_t' if transform else ''}>>h_bkgmc_cen", "weight*%f*((m_mumu>=120&&m_mumu<=130)&&(eventNumber%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
    if estimate in ["fullSimrw"]:
        ...
        t_bkgmc.Draw(f"{variable}_score{'_t' if transform else ''}:{variable}_score_VBF{'_t' if transform else ''}>>h_bkgmc_sid_raw", "weight*%f*((m_mumu>=110&&m_mumu<=180)&&!(m_mumu>=120&&m_mumu<=130)&&(eventNumber%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
        t_bkgmc.Draw(f"{variable}_score_VBF{'_t' if transform else ''}>>h_bkgmc_sid", "weight*%f*((m_mumu>=110&&m_mumu<=180)&&!(m_mumu>=120&&m_mumu<=130)&&(eventNumber%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
    if estimate in ["fullSimrw", "data_sid"]:
        ...
        t_data_sid.Draw(f"{variable}_score{'_t' if transform else ''}:{variable}_score_VBF{'_t' if transform else ''}>>h_data_sid_raw", "weight*%f*((m_mumu>=110&&m_mumu<=180)&&!(m_mumu>=120&&m_mumu<=130)&&(eventNumber%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
        t_data_sid.Draw(f"{variable}_score_VBF{'_t' if transform else ''}>>h_data_sid", "weight*%f*((m_mumu>=110&&m_mumu<=180)&&!(m_mumu>=120&&m_mumu<=130)&&(eventNumber%%%d!=%d))"%(n_fold/(n_fold-1.) if n_fold != 1 else 1, n_fold, fold if n_fold != 1 else 1))
